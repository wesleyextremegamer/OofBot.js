...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('ping')
		.setDescription('responds with with cool bot status info.'),
	/** @param {import('discord.js').ChatInputCommandInteraction} interaction */
	async execute(interaction) {
		async function SendMessage(msgInteraction) {
			...
			async function getWsPing() {
				let ping;
				for (let i = 0; i < 5; i++) {
					ping = msgInteraction.client.ws.ping;
					if (ping || ping >= 0) {
						return ping;
					}
					await wait(50);
			...
			async function getRestPing() {
				...
				await msgInteraction.user.fetch();
			...
			async function UpdateWithRest() {
				...
				const editedResponse = await GetContextReply(msgInteraction, false)({ embeds : [pingEmbed], components : [row] });
				const collector = editedResponse.createMessageComponentCollector({
					componentType : ComponentType.Button,
					time: 15000,
				});
				collector.once('collect', async I => {
					if (!I.user.id === msgInteraction.user.id) {
						return await I.reply({ content: '❌ Oof !\n`You cant interact with this button, sorry man.`', ephemeral: true });
					}
			...
			const upTime = `<t:${Math.floor(msgInteraction.client.readyTimestamp / 1000)}:F>`;
			...
			await GetContextReply(msgInteraction, false)({ embeds : [pingEmbed] });
